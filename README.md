...
[style guide](docs/style-guide.md) on 2026-09-08. M1.3 is complete: the responsive catalog/service shell, product/limits rendering, theme and copy interactions, accessibility, and metadata pass local checks. M1.4 implementation is ready; owner launch is pending. Product research is M2; the tested deploy
...
**Implementation ready; owner launch pending.** Agents never run the deploy
entry point, change plex, commit or push. Review and commit the working tree
before the first deployment. M2 begins after the owner launch check.
...
```sh
ssh plex 'sudo cp -a /etc/nginx/sites-available/devstack.fyi /etc/nginx/sites-available/devstack.fyi.pre-m14'
...
Make the backup once; do not overwrite it on retries. If validation or the
reload fails, restore it before retrying:
...
and TLS/Certbot files are not changed. nginx may serve an error until the first
build is deployed into the currently empty docroot.
...
Record the owner launch result in `docs/plan.md`; that completes M1.
