...
save-future-you log, not a compliance artifact. (Findings about a _service's
behavior_ that belong in the published notes go in that service's content or
...
The M1.1 pnpm 12.3.4 lockfile stores the package-manager executable graph and
project dependency graph in separate YAML documents. `yaml.parse()` throws
`MULTIPLE_DOCS`; reading only one would silently omit packages from the license
audit. Use `parseAllDocuments()`, reject parse errors, and union every document's
package keys. The current inventory includes both graphs and optional platforms.

## RE-008: Temporary Astro projects sharing node_modules also share its default cache (2026-09-08, status: worked-around)

Environment: Astro 7.3.2, real build tests in temporary copies of the project,
with node_modules symlinked to the installed checkout to avoid repeated installs.
The tests passed, but the following main build failed resolving a deferred MDX
module for `services/example/content/local-dev/index.mdx`, a fixture that never
existed in the main checkout. Astro's default node_modules/.astro cache shared
content state across projects. Set `cacheDir: './.astro/cache'` in the project
config so every checkout/fixture has its own cache; remove the already-generated
`.astro/` state once after fixing the configuration. Verify with the full
`pnpm check` followed by `pnpm build`, not tests alone.
